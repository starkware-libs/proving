...
/// Holds one `T` per circuit component, in the committed component order — the order in which the
/// prover commits trace/interaction columns and the verifier evaluates constraints. This is the
/// `ComponentList` declaration order of the `circuit-verifier` crate (matching the field order of
/// its `PerComponent`).
