...
/// multiverifier proving. A 1-leaf tree never verifies the proof, so a dummy inline proof and a
/// placeholder root suffice (the output values are derived from the empty `output_preimage`,
/// never checked against the proof). Gated behind `slow-tests` because it still builds the
...
fn test_single_leaf_passthrough_succeeds() {
    ...
    // flattened in.
    let digest = r#"["0x0","0x0","0x0","0x0","0x0","0x0","0x0","0x0"]"#;
    let leaf_json = format!(
        r#"{{"output_preimage":[],"circuit_preprocessed_root":{digest},"circuit_hash":{digest},"proof":"AQIDBA=="}}"#,
    );
    ...
    assert!(status.success(), "1-leaf passthrough should succeed, got: {status:?}");
    ...
    assert!(dir.join("root_packed.json").exists(), "packed tree file written");
}
