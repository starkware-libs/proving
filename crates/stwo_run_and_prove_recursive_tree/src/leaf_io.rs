...
impl LeafInput {
    ...
    pub fn output_values(&self) -> Result<[u32; N_RESERVED], RecursiveTreeError> {
        ...
        // H1, over the same u32-word encoding Cairo's `encode_felt252_data_and_calc_blake2s`
        // hashes; its 32 digest bytes read as eight little-endian words.
        ...
        Ok(std::array::from_fn(|i| u32::from_le_bytes(h1[i * 4..i * 4 + 4].try_into().unwrap())))
    }
}
...
impl LeafProofExt for SerializedLeafProof {
    fn preprocessed_root(&self) -> HashValue<QM31> {
        HashValue::from(self.circuit_preprocessed_root.0)
    }
}
