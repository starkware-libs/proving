...
impl Channel {
    ...
    ///
    /// Mirrors the out-of-circuit `Blake2sM31MerkleChannel::mix_hash`: the current
    ...
    /// `Blake2s(digest_words || values)`, where `digest_words` is the current digest encoded as
    /// u32s.
    pub fn mix_u32s(
        &mut self,
        context: &mut Context<impl IValue>,
