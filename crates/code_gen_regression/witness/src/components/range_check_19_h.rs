#![allow(unused_parens)]
use cairo_air::components::range_check_19_h::{Claim, InteractionClaim, LOG_SIZE, N_TRACE_COLUMNS};
...
fn write_trace_simd(mults: Vec<PackedM31>) -> (ComponentTrace<N_TRACE_COLUMNS>, LookupData) {
    ...
    (trace.par_iter_mut(), lookup_data.par_iter_mut())
        ...
        .for_each(|(row_index, (mut row, lookup_data))| {
            ...
            *lookup_data.range_check_19_h_0 = [seq];
...
impl InteractionClaimGenerator {
    pub fn write_interaction_trace(
        ...
        logup_gen.col_from_par_iter(
            (&self.lookup_data.range_check_19_h_0, self.lookup_data.mults)
                .into_par_iter()
                .map(|(values, mults)| {
                    let denom = range_check_19_h.combine(values);
