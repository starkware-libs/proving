...
fn test_partial_mul_generic() {
    ...
    let q = P_1;
    let [q_x, q_y] = [
        const_felt252_expr_from_felt252!(q.x),
        const_felt252_expr_from_felt252!(q.y),
    ];

    let p = P_3;
    let [p_x, p_y] = [
        const_felt252_expr_from_felt252!(p.x),
        const_felt252_expr_from_felt252!(p.y),
    ];
    ...
    let q2 = &ec_mul(&q, 2);
    let [q2_x, q2_y] = [
        const_felt252_expr_from_felt252!(q2.x),
        const_felt252_expr_from_felt252!(q2.y),
    ];

    let res = ec_add(&p, &q);
    let [res_x, res_y] = [
        const_felt252_expr_from_felt252!(res.x),
        const_felt252_expr_from_felt252!(res.y),
    ];
    ...
    let q4 = &ec_mul(&q, 4);
    let [q4_x, q4_y] = [
        const_felt252_expr_from_felt252!(q4.x),
        const_felt252_expr_from_felt252!(q4.y),
    ];
    ...
    let (state, output3) = registry.run_air(
        ...
        (
            call_id.clone(),
            const_expr!(26),
            (m_spec, [q_x, q_y], [p_x, p_y], const_expr!(0)),
        ),
    );

    let m_spec_2: Felt252Width27Expr = const_felt252_expr!(
        0xca8642002468acf13579bdffdb97530f,
        0x2468acf13579bdffdb97530e
    )
    .into();
...
fn test_partial_mul_generic_special_round_check() {
    ...
    registry.run_air(
        ...
        (
            call_id.clone(),
            const_expr!(26),
            (m, [q_x, q_y], [p_x, p_y], const_expr!(0)),
        ),
    );
}
...
fn test_ec_op_builtin() {
    ...
    let res_x = const_felt252_expr!(
        0x71e2b508e8181cbb1d1d955e69f8ad85,
        0x6b9fe6322bda8c705fa87d37b75007d
    );
    let res_y = const_felt252_expr!(
        0x103e5924ba3b26345d80a182ed14e859,
        0x6150ca9cae42b6c63f4a694429d0984
    );
