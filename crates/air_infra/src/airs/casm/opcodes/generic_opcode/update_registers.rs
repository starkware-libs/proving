    ...
    fn call(
        ...
        let next_pc = flags[FLAG_PC_UPDATE_REGULAR_INDEX].clone()
            * (casm_state.pc.clone() + flags[INSTRUCTION_SIZE_INDEX].clone())
            ...
            + flags[FLAG_PC_UPDATE_JUMP_REL_INDEX].clone()
                * (casm_state.pc.clone() + res_as_rel_imm.clone())
            + flags[FLAG_PC_UPDATE_JNZ_INDEX].clone() * npc_jnz;

        // Update ap
        let next_ap = casm_state.ap.clone()
        ...
        // Update fp
        let next_fp = flags[FLAG_FP_UPDATE_REGULAR_INDEX].clone() * casm_state.fp.clone()
            + flags[FLAG_OPCODE_RET_INDEX].clone() * dst_as_addr
            + flags[FLAG_OPCODE_CALL_INDEX].clone() * (casm_state.ap.clone() + const_expr!(2));
